          ...
          {def.type === "boolean" ? (
            ...
            <>
              <input
                ...
                className={`bg-[#1B1C21] border rounded-[8px] px-2.5 py-1.5 text-[12px] text-fg outline-none w-[80px] text-right focus:border-accent/50 ${
                  isDirty
    ...
    <div className="flex flex-col gap-1.5">
      ...
      {def.type === "text" && (
        <input
          ...
          className={`bg-[#1B1C21] border rounded-[8px] px-3 py-1.5 text-[12px] text-fg outline-none focus:border-accent/50 ${
      ...
      {def.type === "textarea" && (
        <textarea
          ...
          onChange={(e) => onChange(e.target.value)}
          rows={3}
          className={`bg-[#1B1C21] border rounded-[8px] px-3 py-2 text-[12px] text-fg outline-none resize-y min-h-[64px] leading-relaxed focus:border-accent/50 ${
            isDirty ? "border-[rgba(240,169,43,0.4)]" : "border-border"
          }`}
        />
      )}
      ...
      {def.type === "select" && (
        <select
          value={String(value ?? "")}
          onChange={(e) => onChange(e.target.value)}
          className={`bg-[#1B1C21] border rounded-[8px] px-3 py-1.5 text-[12px] text-fg outline-none focus:border-accent/50 appearance-none bg-no-repeat bg-[right_11px_center] pr-8 cursor-pointer ${
            isDirty ? "border-[rgba(240,169,43,0.4)]" : "border-border"
          }`}
          style={{
